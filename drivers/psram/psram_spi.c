...
uint32_t psram_size() {
    ...
    _res = _psram_size();
    return _res/0x100000; // в Mb
}


uint32_t init_psram() {
    psram_spi = psram_spi_init_clkdiv(PIO_PSRAM, SM_PSRAM, PSRAM_DIV, false);
    if ( !_psram_size() ) {
     //  psram_spi = psram_spi_init_clkdiv(PIO_PSRAM, SM_PSRAM, PSRAM_DIV, true);// чегото там другой способ чтения если psram size = 0
      psram_spi_uninit(psram_spi, false);// деинсталяция программы
    }  
